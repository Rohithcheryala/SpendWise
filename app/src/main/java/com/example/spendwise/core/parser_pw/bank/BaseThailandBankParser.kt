...
abstract class BaseThailandBankParser : BankParser() {
    ...
    override fun extractBalance(message: String): BigDecimal? {
        ...
        val patterns = listOf(
            Regex(
                """(?:Bal|คงเหลือ)\s+(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?)\s*(?:THB|บาท)""",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                """(?:Bal|คงเหลือ)\s+(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?)""",
                RegexOption.IGNORE_CASE
            )
        )
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        val patterns = listOf(
            Regex(
                """(?:at|ร้าน)\s+([A-Za-z0-9\s&._-]+?)(?:\s+(?:A/C|บช|Bal|คงเหลือ|Available|on|$))""",
                RegexOption.IGNORE_CASE
            ),
    ...
    override fun extractAvailableLimit(message: String): BigDecimal? {
        val pattern = Regex(
            """(?:Available limit|วงเงินคงเหลือ)\s+(\d{1,3}(?:,\d{3})*(?:\.\d{1,2})?)\s*(?:THB|บาท)""",
            RegexOption.IGNORE_CASE
        )
        pattern.find(message)?.let { match ->
            val cleanAmount = match.groupValues[1].replace(",", "")
