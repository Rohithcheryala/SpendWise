...
class EmiratesNBDParser : UAEBankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        val toPattern =
            Regex("""to\s+([A-Z][A-Z0-9\s]+?)(?:\s+on|\s+\(|$)""", RegexOption.IGNORE_CASE)
    ...
    override fun extractBalance(message: String): BigDecimal? {
        ...
        val balancePatterns = listOf(
            ...
            Regex(
                """(?:Avl\s+Bal|Available\s+Balance)(?:\s+is)?\s*([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),

            // Pattern 2: "Available Balance: CURRENCY X,XXX.XX"
            Regex(
                """Available\s+Balance:\s*([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            )
        )
    ...
    override fun extractAvailableLimit(message: String): BigDecimal? {
        ...
        val limitPatterns = listOf(
            ...
            Regex(
                """Avl\s+Cr\.?\s+Limit(?:\s+is)?\s*([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),

            // Pattern 2: "Available Credit Limit: CURRENCY X,XXX.XX"
            Regex(
                """Available\s+Credit\s+Limit:\s*([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            )
        )
