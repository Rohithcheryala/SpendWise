...
class SliceParser : BankParser() {
    ...
    private fun isDatePhrase(text: String): Boolean {
        ...
        val datePattern = Regex(
            """\b(?:\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)|(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+\d{1,2})\b""",
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        val sentToPattern =
            Regex("""sent.*to\s+([A-Z][A-Z0-9\s./&-]+?)\s*\(""", RegexOption.IGNORE_CASE)
        ...
        val onPattern =
            Regex("""\bon\s+([A-Za-z0-9\s./&-]+?)(?:\s+is|$)""", RegexOption.IGNORE_CASE)
