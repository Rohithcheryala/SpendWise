...
class TelebirrParser : BankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        val bankFromPattern =
            Regex("""from\s+([A-Za-z\s]+Bank)\s+to\s+your""", RegexOption.IGNORE_CASE)
        ...
        paidToPattern.find(message)?.let { match ->
            ...
            if (matchEnd < message.length) {
                ...
                if (textAfterMatch.trimStart()
                        .startsWith("on ", ignoreCase = true) && textAfterMatch[0] == ' '
                ) {
                    merchant += " "
                }
            }
        ...
        val purchasedFromPattern = Regex(
        ...
        val packagePattern = Regex(
            """for\s+package\s+([^,\n]+?)(?:\s+purchase\s+made|\s+on\s+\d{2}/\d{2}/\d{4}|\.\s+Your\s+transaction|$)""",
            RegexOption.IGNORE_CASE
        )
        packagePattern.find(message)?.let { match ->
            var merchant = match.groupValues[1].trim()
            // Also capture "purchase made for X" part if it exists
            val purchaseMadePattern =
                Regex("""purchase\s+made\s+for\s+(\d+)""", RegexOption.IGNORE_CASE)
        ...
        val transferredToPattern = Regex(
            """transferred\s+[^,\n]+?\s+to\s+([^,\n]+?)(?:\s+on\s+\d{2}/\d{2}/\d{4}|\.|$)""",
            RegexOption.IGNORE_CASE
        )
        ...
        val fromPattern = Regex(
            """from\s+(?!your\s+account)([^,\n]+?)(?:\s+on\s+\d{2}/\d{2}/\d{4}|\s+to\s+your|\.|$)""",
            RegexOption.IGNORE_CASE
        )
    ...
    override fun extractBalance(message: String): BigDecimal? {
        ...
        val eMoneyBalancePattern =
            Regex(
                """E-Money Account\s+balance is ETB\s+([0-9,]+(?:\.[0-9]{2})?)""",
                RegexOption.IGNORE_CASE
            )
        ...
        val telebirrBalancePattern =
            Regex(
                """telebirr account balance is\s+ETB\s+([0-9,]+(?:\.[0-9]{2})?)""",
                RegexOption.IGNORE_CASE
            )
    ...
    override fun extractReference(message: String): String? {
        ...
        val bankTransactionPattern =
            Regex("""bank transaction number is\s+([A-Z0-9]+)""", RegexOption.IGNORE_CASE)
        bankTransactionPattern.find(message)?.let { match ->
            return match.groupValues[1]
        }

        // Look for "by transaction number DAV5COORPD" (bank transfer income)
        val byTransactionPattern =
            Regex("""by transaction number\s+([A-Z0-9]+)""", RegexOption.IGNORE_CASE)
        ...
        val transactionPattern =
            Regex("""(?:your\s+)?transaction number is\s+([A-Z0-9]+)""", RegexOption.IGNORE_CASE)
