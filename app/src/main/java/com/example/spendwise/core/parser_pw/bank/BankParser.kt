...
abstract class BankParser {
     ...
     */
    private fun isValidAccountLast4(
        last4: String,
        matchedText: String,
        fullMessage: String
    ): Boolean {
        ...
        val datePatterns = listOf(
            ...
            Regex(
                """\bon\s+\d{1,2}[/-]\d{1,2}[/-]$escapedLast4""",
                RegexOption.IGNORE_CASE
            ),  // "on 04/11/2025"
            Regex(
                """\bdated\s+\d{1,2}[/-]\d{1,2}[/-]$escapedLast4""",
                RegexOption.IGNORE_CASE
            )  // "dated 05-02-2025"
        )
    ...
    protected open fun extractAvailableLimit(message: String): BigDecimal? {
        ...
        val creditLimitPatterns = listOf(
            ...
            Regex(
                """Available\s+limit:?\s*$cur\s*([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
            ...
            Regex(
                """Available\s+Credit\s+Limit:?\s*$cur\s*([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
