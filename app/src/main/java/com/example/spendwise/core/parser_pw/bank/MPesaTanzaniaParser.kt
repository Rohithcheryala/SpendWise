...
class MPesaTanzaniaParser : BankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        Regex("""repayment of\s+(.+?)\s+service""", RegexOption.IGNORE_CASE).find(message)
            ?.let { match ->
        ...
        Regex(
            """sent to business\s+(.+?)(?:\s+on\s+\d|\s+for\s+account|\s+Total\s+fee|$)""",
            RegexOption.IGNORE_CASE
        )
        ...
        Regex("""sent to\s+(.+?)\s+for\s+account""", RegexOption.IGNORE_CASE).find(message)
            ?.let { match ->
                val merchant = cleanTzMerchant(match.groupValues[1])
                if (isValidMerchantName(merchant)) {
                    return merchant
        ...
        Regex(
            """paid to\s+(.+?)(?:\s+for\s+account|\s+on\s+\d|\s*\(Merchant|\s+and\s+charged|$)""",
            RegexOption.IGNORE_CASE
        )
            .find(message)?.let { match ->
                val merchant = cleanTzMerchant(match.groupValues[1])
        ...
        Regex(
            """received\s+a\s+payment\s+of\s+$currencyToken\s*[0-9,]+(?:\.[0-9]{1,2})?\s+from\s+(.+?)(?:\s+on\s+\d|$)""",
            RegexOption.IGNORE_CASE
        )
            .find(message)?.let { match ->
        ...
        Regex(
            """received\s+$currencyToken\s*[0-9,]+(?:\.[0-9]{1,2})?\s+from\s+(.+?)(?:\s*\(|\s+on\s+\d|$)""",
            RegexOption.IGNORE_CASE
        )
            .find(message)?.let { match ->
                val merchant = cleanTzMerchant(match.groupValues[1])
    ...
    private fun cleanTzMerchant(raw: String): String {
        return raw
            ...
            .replace(
                Regex("""\s+tarehe\s+\d.*""", RegexOption.IGNORE_CASE),
                ""
            ) // Swahili date suffix
