...
open class FABParser : UAEBankParser() {
    ...
    override fun extractAmount(message: String): BigDecimal? {
        ...
        val patterns = listOf(
            Regex(
                """funds transfer request of\s+(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                """for\s+(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
            Regex(
            ...
            Regex(
                """Amount\s*(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+\*([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                """Amount\s*(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+([0-9*,]+(?:\.\d{2})?)""",
            ...
            Regex(
                """payment.*?(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+\*([0-9,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
            Regex(
                """payment.*?(${CompiledPatterns.Currency.ISO_CODE.pattern})\s+([0-9*,]+(?:\.\d{2})?)""",
