...
class LivBankParser : UAEBankParser() {
    ...
    override fun extractMerchant(message: String, sender: String): String? {
        ...
        if (lowerMessage.contains("purchase of")) {
            ...
            val merchantPattern =
                Regex("""at\s+([^,]+?)(?:,|\s+Avl|\.\s)""", RegexOption.IGNORE_CASE)
    ...
    override fun extractAccountLast4(message: String): String? {
        ...
        val cardPattern =
            Regex("""(?:Debit|Credit)\s+Card ending\s+(\d{4})""", RegexOption.IGNORE_CASE)
    ...
    override fun extractBalance(message: String): BigDecimal? {
        ...
        val balancePatterns = listOf(
            ...
            Regex(
                """Current balance is\s+([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),

            // "Avl Balance is CURRENCY X,XXX.XX"
            Regex(
                """Avl Balance is\s+([A-Z]{3})\s+([\d,]+(?:\.\d{2})?)""",
                RegexOption.IGNORE_CASE
            ),
